...
describe("time.ts", () => {
  ...
  describe("split()", () => {
    it("logs split with label + meta, formats ms < 100 with 1 decimal, and logs meta on the next line", () => {
      ...
      // then splitImpl: now()
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([1000, 1034.56]));
    ...
    it("supports split(id, meta) (no label) and logs meta", () => {
      ...
      // then splitImpl: now()
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([2000, 2100]));
    ...
    it("updates lap time across consecutive splits (lastMs moves forward)", () => {
      ...
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([1000, 2500, 3700]));
    ...
    it("formats minutes and hours", () => {
      ...
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([0, 123_210]));
      const tm = time.start(idM, loggerM);
      tm("min");
      expect((loggerM.mock.calls[1]![0] as string)).toContain("[2m 03.21s | +2m 03.21s]");

      perfNowSpy.mockRestore();

      // Hours case:
      // start 0, split at 1h 5m => 3900s => 3_900_000ms
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([0, 3_900_000]));
  ...
  describe("end()", () => {
    ...
    it("supports end(id, meta) and end(id, label, meta) return values", () => {
      ...
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([0, 50, 100, 250]));
  ...
  describe("instance call signature", () => {
    ...
    it("timer() records a split with no label/meta", () => {
      ...
      perfNowSpy = spyOn(performance, "now").mockImplementation(seq([1000, 1123]));
